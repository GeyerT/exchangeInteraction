...
class KrakenClient extends BasicClient {
  ...
  */
  constructor(authorization = undefined, { wssPathPub = "wss://ws.kraken.com", wssPathPriv = "wss://ws-auth.kraken.com", autoloadSymbolMaps = true, watcherMs } = {}) {
    let wssPath = (authorization === undefined) ?
        wssPathPub
        :
        wssPathPriv
    ...
    if (this.hasAuthorization === true)
    {
      this.authorization = authorization
      if (this.authorization !== undefined)
      {
        this.token = this._getPrivateToken()
      }
    }
  ...
  /**
    ...
    JSON-RPC call
   */
  _debounceSend(debounceKey, subMap, subscribe, subscription, type) {
    /*

    this._debounceSend("sub-orders", this._ordersSubs, true, {
      name: "ownTrades",
      token: this.token,
    });
  ...
  _getAuthorization() {
    if (this.hasAuthorization === true)
    {
      if (this.authorization !== undefined)
      {
        if ("secretKey" in this.authorization &&
            "apiKey" in this.authorization)
        {
          return true
        }
      }
  ...
  async _getPrivateToken() {
    let baseURL = "https://api.kraken.com"
    let endpointExchange = "/0/private/GetWebSocketsToken"
    let url = baseURL + endpointExchange

    let nonce = Date.now()
    let raw = await axios.post(url, "nonce="+nonce, this._sign(endpointExchange, nonce, "POST"))

    if (!raw.data.result || !raw.data.result.token) this._getPrivateToken();
    else this.token = raw.data.result.token
  }

  _sign (endpoint, params, method) {

    const nonce = params
    const message = 'nonce='+nonce;
    const secret_buffer = new Buffer.from(this.authorization.secretKey, 'base64');
  ...
  async _sendUnsubOrders() {
    ...
    this._debounceSend("unsub-orders", this._ordersSubs, false, {
      name: "openOrders",
      token: this.token,
     });
  }
  ...
  _processMessage(msg) {
    ...
    if (msg.event === "subscriptionStatus") {
      ...
      */
      ...
      if ("channelID" in msg) this.subscriptionLog.set(parseInt(msg.channelID), msg);
      else this.subscriptionLog.set(msg.channelName, msg);
    ...
    let [subscriptionId, details] = (msg.length === 3) ?
        [msg[1], msg[0]]
        :
        msg
    ...
    if (sl.subscription.name === "openOrders") {
      ...
      {
        for (let orderId in details[key])
        {
          let orders = new Orders({
            exchange: "Kraken",
            symbol: ("descr" in details[key][orderId]) ? details[key][orderId].descr.pair : undefined,
            orderType: ("descr" in details[key][orderId]) ? details[key][orderId].descr.ordertype : undefined,
            side: ("descr" in details[key][orderId]) ? details[key][orderId].descr.type : undefined,
            orderId: orderId,
            type: ("descr" in details[key][orderId]) ? details[key][orderId].descr.type : undefined,
            time: ("opentm" in details[key][orderId]) ? details[key][orderId].opentm : undefined,
            size: ("vol" in details[key][orderId]) ? details[key][orderId].vol : undefined,
            filledSize: ("vol_exec" in details[key][orderId]) ? details[key][orderId].vol_exec : undefined,
            price: ("descr" in details[key][orderId]) ? details[key][orderId].descr.price : undefined,
            remainSize: undefined,
            status: ("status" in details[key][orderId]) ? details[key][orderId].status : undefined,
            timestamp: undefined
          })
