...
class KucoinClient extends BasicClient {
   ...
   * subscriptions.
   */
  constructor(authorization = undefined, { wssPath, watcherMs, sendThrottleMs = 10, restThrottleMs = 250 } = {}) {
    ...
    if (this.hasAuthorization === true)
    {
      if (this.authorization !== undefined)
      {
        if ("secretKey" in this.authorization &&
            ...
            "passphrase" in this.authorization)
        {
          return true
        }
      }
  ...
  _sign (endpoint, params, method) {
    ...
    let nonce = Date.now() + ''
    let strForSign = nonce + method + endpoint + JSON.stringify(params)

    let signatureResult = crypto.createHmac('sha256', this.authorization.secretKey)
      .update(strForSign)
      .digest('base64')
    let passphraseResult = crypto.createHmac('sha256', this.authorization.secretKey)
      .update(this.authorization.passphrase)
      .digest('base64')

    header.headers['KC-API-SIGN'] = signatureResult
    header.headers['KC-API-TIMESTAMP'] = nonce
    header.headers['KC-API-KEY'] = this.authorization.apiKey
  ...
  async _connectAsync() {
    ...
    let authorized = this._getAuthorization()
    ...
    while (!wssPath) {
      ...
      {
        try {
          let baseURL = "https://openapi-v2.kucoin.com"
          let endpointExchange = '/api/v1/bullet-private'
          let url = baseURL + endpointExchange

          let raw = await axios.post(url, {}, this._sign(endpointExchange, {}, "POST"))
          if (!raw.data.data || !raw.data.data.token) throw new Error("Unexpected token response");
          const { token, instanceServers } = raw.data.data;
  ...
  _processOrders(msg) {
    let { symbol, orderType, side, orderId, type, orderTime, size, filledSize, price, remainSize, status, ts } = msg.data;
    ...
    let orders = new Orders({
      ...
      price: price,
      remainSize: (parseFloat(size)-parseFloat(filledSize)).toString(),
      status: status,
      timestamp: ts
    });
  ...
  _processL2Update(msg) {
    ...
    console.log(msg.data, parseFloat(msg.data.sequenceStart) - parseFloat(msg.data.sequenceEnd))
