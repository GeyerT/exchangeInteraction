...
class BasicTradeClient extends EventEmitter {
  ...
  subscribeOrders() {
    if (!this.hasOrders && !this.hasAuthorization) return;
    return this._subscribe("all", this._ordersSubs, this._sendSubOrders.bind(this));
  }

  unsubscribeOrders() {
    if (!this.hasOrders && !this.hasAuthorization) return;
    this._unsubscribe("all", this._ordersSubs, this._sendUnsubOrders.bind(this));
  }
  ...
  subscribeBBO(market) {
    if (!this.hasBBO) return;
    return this._subscribe(market, this._bboSubs, this._sendSubBBO.bind(this));
  }

  unsubscribeBBO(market) {
    if (!this.hasBBO) return;
    this._unsubscribe(market, this._bboSubs, this._sendUnsubBBO.bind(this));
  }
  ...
  _subscribe(market, map, sendFn) {
    this._connect();
    let remote_id = (!isObject(market)) ? market : ("symbol" in market) ? market.symbol : market.name;
  ...
  _onConnected() {
    ...
    for (let [marketSymbol, market] of this._ordersSubs) {
      this._sendSubOrders()
    }
    for (let [marketSymbol, market] of this._bboSubs) {
      this._sendSubBBO(marketSymbol, market)
    }
    this._watcher.start();
  }
